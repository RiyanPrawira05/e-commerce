...
<div class="row">
    <div class="col-md-12">
        <div class="card shadow">
            ...
            <form class="horizontal" action="{{ Route('Pcolors.update', $productwarna->id_product_warna) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="col-md-12">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                @foreach($warna as $color)
                                    <option value="{{ $color->id_warna }}" {{ $color->id_warna == $productwarna->warna ? 'selected' : '' }}>{{ $color->warna }}</option>
                                @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                @foreach ($product as $products)
                                    <option value="{{ $products->id_product }}" {{ $products->id_product == $productwarna->product ? 'selected' : '' }}>{{ $products->product }}</option>
                                @endforeach
                                </select>
                            </div>
                        </div>
